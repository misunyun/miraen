//http://localhost:8822/Ebook/preview.mrn?source=TEXTBOOK&file=bf91aa00858d4a3f8bd1feee1e855599&contentsTextbookUnitScrapYn=Y&contentsTextbookTextbookSeq=1
...
var viewer = {
    ...
    check_parameters: function () {
        ...
        this.mType = 'mtype' in queryParams ? queryParams.mtype : undefined; 	// 23.2.10 mtype : HLS 로 오는 경우의 param 추가
        																		// todo : hls의 경우 검증 조건 추가해야함.
																				//console.log(queryParams); 23.2.10 lsj xxx
    ...
    ready: function (_cb) {
        ...
        this.check_parameters()
            ...
            .then(function () {
                ...
                if(self.mType === 'HLS') {
					return tms.get_hls_info(self.file_id);
				}
				else if(self.mType === 'YOUTUBE') {
					console.log('YOUTUBE');

                	// return tms.get_file_info_seq(self.contentsTextbookPeriodSeq);
				}
				else if(self.mType === 'LINK') {
					console.log('LINK');
					...
					console.log('FILE');
					return tms.get_file_info(self.file_id);
				}
            })
            .then(function (data) {
                // console.log(data);
                ...
                /** 23.2.10 lsj xxx
                동영상으로 가져오는 경우, 기존의 fileId를 조회해서 가져오는 부가 정보가 많이 있으나
                HLS 타입으로 조회해서 가져오는 경우엔 부가정보가 없어 에러가 발생하는 부분이 많이 있습니다.
                 */


		        /* // 부가정보가 없어서 hls로 임시 분기했던 부분 23.2.10 lsj xxx
		        if(self.mType === 'HLS') {

			        console.log('uppder hls');
			        console.log('uppder hls');
				...
				if (data && data.type && (data.type == 'zip' || data.type == 'ZIP')) {
                    ...
                    let userId =  $('body').data('userid') || ''; // https://mteacher.dooray.com/project/posts/3543729016882654035 예외처리
                    if (!tms._user && userId != "2600434") {
                    ...
                    /** 23.2.10 lsj	부가정보 필요한 부분. (utils.js 277 line) xxx
                    data.code      if (!data || data.code && data.code == "fail") {
                    data.type      if (data && data.type && (data.type == 'zip' || data.type == 'ZIP')) {
                    data.pages     if (!data.pages || data.pages.length <= 0) {
                     */
                }
    ...
    set_info: function (data, _cb) {
        ...
        if(self.mType === 'HLS') {
			// 23.2.10 lsj 이 부분에서 hls type, url 입력하면 됩니다. xxx
        ...
        setTimeout(function () {
            ...
            if (self.file_type == 'video' || self.file_type == 'audio') {
				...
				// 아래  Module.video_viewer 는  /assets/viewer/js/viewer.module.js 에 있습니다.
				Module.video_viewer.playlist(data.videoUrl, 'video/hls');
