; (function ($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    init: function () {
      ...
      // 여기서 popover 에 넓이를 변경해서 horizontal 로 뿌려줘야함.
      // 지금 로직은 입력 부분의 넓이를 그대로 사용하기 때문에.
      ...
      this.addClassNameListener(this.$elem, function(lastClassName, currentClassName) {
        $this.log('tagWrapper className before : ', $this.$tagWrapper.attr('class'));
        $this.$tagWrapper.removeClass(lastClassName);
        $this.$tagWrapper.addClass(currentClassName);
        $this.log('lastClassName : ', lastClassName);
        $this.log('currentClassName : ', currentClassName);
        $this.log('tagWrapper className after : ', $this.$tagWrapper.attr('class'));
      });
    ...
    addClassNameListener: function($elem, callback) {      
      var lastClassName = $elem.attr('class');
      window.setInterval( function() {
         var className = $elem.attr('class');
          if (className !== lastClassName) {
              callback(lastClassName, className);
    ...
    popoverToInput: function ($elem) {
      ...
      if (arrValues.indexOf(val) !== -1) {
        // 중복 입력은 피한다.
        return;
      } else {
        // 다른 값이 들어왔을 경우 교체 동작을 한다.
        if (this.$tags.find(addDot(classNames.tag)).length > 0) {
          this.$tags.find(addDot(classNames.tag)).remove();
        }
        this.$tags.append(li);

        // 원본 select 와 값을 동기화 시킴.
