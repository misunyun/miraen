...
var screenMode = 'landscape';/*'portrait' | 'landscape'*/
var diviceMode = 'tablet';
...
function setTitle(){
    var sBookName = decodeURI(decodeURIComponent(getParam("book_name")));
    var sTitle = decodeURI(decodeURIComponent(getParam("title")));

    var title_btn = document.getElementById("title_btn")
    var title = document.getElementById("title")

    if(title_btn) title_btn.innerHTML = sBookName;
    if(title) title.innerHTML = sTitle;
...
//    var token = getParam("token");
    ...
    const modalCancel = document.getElementById('cancel_btn');
...
function showMarking(divNum){
    ...
    var marking = document.getElementById("marking");
	var scaleablePaddingTop = parseInt(styles.getPropertyValue('--scaleable-padding-top'));
    ...
    try{
     ...
     console.log("review", currentPage, review);
...
//    setQuestionEnable( "false" );
    ...
    // 가로모드 채점 위치 수정
	var spt = screenMode == 'portrait' ? 0 : scaleablePaddingTop;
    marking.style.left = markingStartX + "px";
    marking.style.top = startY + spt + "px";
...
function showEachScore(isCorrect) {
  if (isCorrect) {
    //        modalCorrect(isCorrect);
//    alert('정답');
    arrQuestion[currentPage]['review'].isCorrect = true;
    showQuestionNum(currentPage);
  } else {
    //        modalCorrect(isCorrect);
//    alert('오답');
    arrQuestion[currentPage]['review'].isCorrect = false;
    showQuestionNum(currentPage);
  }
...
function getEachScore(isShowAddClss = 'false') {
  try {
    console.log('getEachScore currentPage: ' + currentPage);
    ...
    // }
    if (arrQuestion[currentPage].review == null) {
      arrQuestion[currentPage].review = {};
      arrQuestion[currentPage].review.retryCount = 0;
    } else {
      ++arrQuestion[currentPage].review.retryCount;
    }

    arrQuestion[currentPage].mode = 'review';
    console.log('arrQuestion[currentPage]', arrQuestion[currentPage]);
    var scoreService = ps.getScore();
    currentPage = ps.getCurrentPageIndex(); //get current
    ...
    if (page.isReportable()) {
      ...
      if (
        (score['score'] == score['maxScore'] &&
          (score['errorCount'] == 0 || score['errorCount'] >= 100)) ||
        score['errorCount'] == 999
      ) {
    ...
    //        console.log("getEachScore",e, currentPage);
    //        showEachScore(false);
    return false;
  }
}
...
function getAnswerSolve() {
  const container = document.querySelector('#_QPlayer');
  var variableStorages = container.querySelectorAll(
    '[class=addon_Variable_Storage]'
  );
  ...
  return {
    ...
    solveType: solveType,
    answerQnote: answerQnote,
    solveQnote: solveQnote,
  };
}
...
var scaleablePaddingTop = parseInt(
  styles.getPropertyValue('--scaleable-padding-top')
); //42;
...
function doJimunPosision() {
  ...
  questionBottom =
    arrQuestion != null && arrQuestion[currentPage].mode == 'review' ? 80 : 30;
  ...
  var menuHeight = parseInt(styles.getPropertyValue('--menu-height'));
  menuHeight *= 1 / wrapperScale;
  // console.log("padding", padding);
  var ich = 0; //$('#_QPlayer').height() + 100;
  // var h = parseInt( (screenHeight - padding) * (1/wrapperScale) );
  var h = parseInt((screenHeight - padding) * (1 / wrapperScale));
  h = Math.max(h, ich);
  console.log('AAAA', h, menuHeight);
  h -= menuHeight;
  console.log('AAAA2', h);
...
function detectMobileDevice(agent, withSize = true) {
  ...
  var bMobile =
    mobileRegex.some((mobile) => agent.match(mobile)) ||
    md.mobile() != null ||
    md.phone() != null ||
    md.tablet() != null;
  console.log("detectMobileDevice4", window.screen.width);
  console.log("detectMobileDevice4", bMobile && (withSize && window.screen.width < 768));

  return bMobile && (withSize && window.screen.width < 768);
}
...
function setScaleViewer() {
  ...
  var standardWidth =
    screenMode === 'portrait'
      ? parseInt(styles.getPropertyValue('--scaleable-width'))
      : parseInt(styles.getPropertyValue('--scaleable-pc-width')); //$("#viewer_container").width();
  var standardHeight = parseInt(styles.getPropertyValue('--scaleable-height'));
  var menuHeight = parseInt(styles.getPropertyValue('--menu_height'));
  var wrapper = $('#scaleable-wrapper');
  var body = $('#body');

  wrapper.removeClass('one-item');

  if (screenMode === 'portrait') {
    standardWidth = parseInt(styles.getPropertyValue('--scaleable-width'));
  } else {
    $('#scaleable-wrapper').addClass('landscape');
    if (pageNum[currentPage] == 1) {
      wrapper.addClass('one-item');
      standardWidth = parseInt(styles.getPropertyValue('--scaleable-width'));
    } else {
      wrapper.removeClass('one-item');
      standardWidth = parseInt(styles.getPropertyValue('--scaleable-pc-width'));
    }
  }
  ...
  var screenSizes =
    window.mAuthor.ScreenUtils.getScreenSizesDependingOnOrientation(userAgent);
  ...
  //    standardHeight = standardHeight * wrapperScale + (78*2);
  ...
  document.getElementById('div_top_padding').style.paddingTop =
    paddingTop + 'px';
  ...
  console.log(isChoco());
  if (arrQuestion[currentPage]?.review != undefined) {
    showMarking();
  } else {
    wrapper.css({ display: 'flex' });
    $('#viewer-top').hide();
  }
  var scale = 1 / wrapperScale;
	var bot = window.innerWidth > 1024 ? 20 : 110 * scale
	document.querySelector('#popup_solve').style.bottom = bot + "px";
}
...
function loadJimun(jimunUrl, questionUrl) {
  ...
  var url =
    'https://miraen-contents.s3.ap-northeast-2.amazonaws.com/cp/qnote/%s/pages/main.xml';
  if (window.location.pathname.split('/')[1] == 'dev') {
    url =
      'https://miraen-contents.s3.ap-northeast-2.amazonaws.com/dev/%s/pages/main.xml';
  } else if (window.location.pathname.split('/')[1] == 'cp') {
    url =
      'https://miraen-contents.s3.ap-northeast-2.amazonaws.com/cp/qnote/%s/pages/main.xml';
  } else if (window.location.pathname.split('/')[1] == 'real') {
    url =
      'https://miraen-contents.s3.ap-northeast-2.amazonaws.com/real/%s/pages/main.xml';
...
function loadQuestion(arrQues) {
  ...
  for (var i = 0; i < arrQues.length; ++i) {
    arrQuestion.push({ url: arrQues[i], review: null, mode: '' });
  }
  ...
  player.load2(questionUrl, 'MATH', '1', quesNum);
        ...
        if (
          ...
          try {
            ...
            window.location.href =
              'toapp://showHandWriting?ishandwriting=' +
              ishandwriting +
              '&maxlength=' +
              maxlength;
          }
  ...
  player.onPageLoaded(function onPageLoadedHandler() {
    ...
    console.log('onPageLoaded', currentNum, currentPage);

    var totalCnt = getQuesTotalCnt();

    console.log('totalCnt|currentNum')
    console.log(totalCnt)
    console.log(currentNum)
    if(currentNum === 0 ){
        document.querySelector('#btn_prev').classList.add('disabled');
    }else{
        document.querySelector('#btn_prev').classList.remove('disabled');
    }
    if(currentNum === totalCnt -1 ){
        document.querySelector('#btn_next').classList.add('disabled');
    }else{
        document.querySelector('#btn_next').classList.remove('disabled');
    }

    showQuestionNum(currentNum);
    showSolveBtn();
    ...
    if (!firstPageLoaded) {
      setJimun(currentPage);
      //initJimunLayout();
      semiResponsiveLayoutChooser = new window.semiResponsive.LayoutChooser(
        player.getSemiResponsiveLayouts()
      );
      var screenSizes =
        window.mAuthor.ScreenUtils.getScreenSizesDependingOnOrientation(
          userAgent
        );
    ...
    (function () {
      ...
      $(document).ready(function () {
        if (scrollDetectorInterval == null) {
          scrollDetectorInterval = setInterval(
            scrollAddIfBodyBiggerThanWindow,
            1000
          );
        }
                  ...
                  if (isShapeOption($option) === true) {
                    ...
                    if (value === '-') {
                      ...
                      $id
                        ...
                        .find('p')
                        .attr('class', '')
                        .addClass(getTextCharCssClass(value));
                    ...
                    if (value === '-') {
                      ...
                      if (
                        $(mutation.target.selectedOptions.item(0)).find('img')
                          .length > 0
                      ) {
                        if ($(p).find('img').length == 0)
                          $(p).append(
                            $(mutation.target.selectedOptions.item(0))
                              .find('img')
                              .clone()
                          );
                        return;
        ...
        function selectChange() {
          ...
          $select.each(function () {
            ...
            $(this).after(
              '<div class="selectChoice"><p></p><div class="inlineChoiceList"></div></div>'
            );
                ...
                if (idx > 0) {
                  ...
                  if ($(this).find('img').length > 0) {
                    ...
                    $(this)
                      ...
                      .next()
                      .find('.inlineChoiceList')
                      .append('<button id=' + id + '>' + $val + '</button>');
                  }
            ...
            if (isShapeOption($option) === true) {
              // console.log("AAAAAA1");
              var $choiceListW =
                $choice.find('.inlineChoiceList').outerWidth() / 2;
              ...
              $option.each(function () {
                ...
                $choice
                  .find('.inlineChoiceList button')
                  .eq(index - 1)
                  .addClass(getBtnCharCssClass(char));
              });
              ...
              if ($pageW < $choListW) {
                ...
                for (var i = 0; i < $btnlength; ++i) {
                  console.log(
                    "$choice.find('.inlineChoiceList >button').eq(i)",
                    $choice.find('.inlineChoiceList >button').eq(i)
                  );
                  var $btnW = $choice
                    .find('.inlineChoiceList >button')
                ...
                console.log(
                  'rowHeight',
                  rowHeight,
                  $choice.find('.inlineChoiceList').attr('rowHeight')
                );
              } else {
              ...
              $('.selectChoice >p').on('click', function () {
                ...
                $(this)
                  .next('.inlineChoiceList')
                  .css({
                    'z-index': '100',
                    opacity: 1,
                    height: listHeight + heightGap,
                  })
                  .show();
              });
          ...
          $('.inlineChoiceList >button').on('click', function (e) {
            ...
            if (isShapeChar($val) === true) {
              ...
              $(this)
                ...
                .prev('p')
                .attr('class', '')
                .addClass(getTextCharCssClass($val));
            }
            ...
            // console.log("ep", ep);
            var isMathJax =
              $('#' + clickBtnID).children('.MathJax_Preview').length > 0;
        ...
        function buttonLimitedReset() {
          if ($('.ic_button_limited_reset').length == 0) {
            return;
          }
    ...
    // 서버 문항정보 로드
    let isNewQnoteServer =
      window.document.referrer.indexOf('https://qnote.miraenchoco.com') > -1 ||
      window.document.referrer.indexOf('https://devqnote.miraenchoco.com/') > -1
        ? true
        : false;
...
function isChangeCurrentQuesInfo() {
  for (var key in objCurrentQuesInfo) {
    if (objCurrentQuesInfo[key] != '' && objCurrentQuesInfo[key] != '0')
      return true;
  }
...
function showSolveBtn(isForce) {
  try {
    var btn = document.getElementById('btn_hint');
    var btnCheck = document.getElementById('btn_check');

    if (arrQuestion == null) return;

    var isShow = arrQuestion[currentPage].mode == 'review';
...
// 초코 컨테이너 초기화 완료 이벤트
function onReady() {
  console.log('onReady from choco');
}
...
function onMessage(receiveData) {
  console.log('choco onMessage from choco', receiveData);
  ...
  switch (receiveData.action) {
    case '@sendURL':
      ...
      if (!!receiveData.data.contentList) {
        receiveData.data.contentList.forEach((item) => {
    ...
    case '@move':
      moveQuestion(receiveData.data.contentId);
      break;
    case '@diviceCheck':
      //  디바이스 변경시 확인
      screenMode = receiveData.data.useDivce;
      diviceMode = receiveData.data.useMode;
      break;
  }
}
...
function getQuestionUrl(contentList) {
  ...
  for (var i = 0; i < contentList.length; ++i) {
    ...
    if (contentList[i].text != undefined)
      passageUrl.push(contentList[i].text.url);
...
var bIsChoco;
function isChoco() {
  // return ("parentChoco" in window);

  if (bIsChoco == undefined) {
    bIsChoco = 'parentChoco' in window;
  }
  return bIsChoco;
}
...
function sendMessageToContainer(action) {
  ...
  switch (action) {
    case '@open':
      ...
      var data = {
        action: '@open',
        data: { contentId: sContentId, hasNext: bHasNext },
      };
      console.log('choco sendMessageToContainer open', data);
      window.parentChoco.sendMessage(JSON.stringify(data));
      break;
    case '@saveTempState':
      ...
      console.log('choco sendMessageToContainer', data);
      window.parentChoco.sendMessage(JSON.stringify(data));
      break;
    case '@saveState':
      var userState = getCurrentQuestionState();
...
function initJimunLayout(flag) {
  ...
  if (
    (diviceMode == 'tablet' || diviceMode == 'mobile') &&
    screenMode === 'portrait'
  ) {
    if (pageNum[currentPage] == 2) {
      var h = $('#viewer_container').height();
      console.log('wrapperScale');
      console.log(wrapperScale);
      $('#viewer_container')
        .height(1000)
        .split({ position: '45%', scale: wrapperScale });
    }
  }
}
...
function autoCheckAndStopWhenWrongLogic() {
  ...
  setTimeout(function () {
    ...
    setTimeout(function () {
      if (
        $('#_QPlayer').find('[class*=wrong]').length <= 0 &&
        _serverArrayData.length > currentNum + 1
      ) {
        autoCheckAndStopWhenWrongLogic();
      } else {
        window.alert = originAlert;
...
function getFileFolder(fileName) {
  ...
  if (arrFolders[0].length != 12) {
    ...
    arrFolders[0] = arrFolders[0].replace(
      /(\w{1})(\w{3})(\d{2})(\d{4})(\d{2})/,
      '$1/$2/$3/$4/$5'
    );
...
function questionInfoDataSet() {
  ...
  var cmsUrl =
    window.location.pathname.split('/')[1] == 'dev'
      ? 'https://cms.miraenchoco.com'
      : window.location.pathname.split('/')[1] == 'cp'
      ? 'https://devcms.miraenchoco.com'
      : 'https://devcms.miraenchoco.com';
  var originPath =
    window.location.pathname.split('/')[1] == 'dev'
      ? 'https://qnote.miraenchoco.com'
      : //( window.location.pathname.split('/')[1] == 'cp' ? 'https://devqnote.miraenchoco.com' : 'http://localhost:8080');
      window.location.pathname.split('/')[1] == 'cp'
      ? 'https://devqnote.miraenchoco.com'
      : 'https://devqnote.miraenchoco.com';
  var options = {
    origin: originPath,
    path: '/authCheck.html',
  ...
  if (
    response_key1 == null ||
    response_key1 == undefined ||
    response_key2 == null ||
    response_key2 == undefined
  ) {
    alert('저작도구 로그인 연결이 끊어 졌습니다.');
    return;
  }
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data.ret == '200') {
        ...
        $('#difficulty').val(contentsInfo.DIFFICULTY).prop('selected', true);
        $('#question_type')
          .val(contentsInfo.QUESTION_TYPE)
          .prop('selected', true);
      }
    },
    error: function (data) {
...
$(document).ready(function () {
  ...
  function getDetailFromHover(e) {
    if (!checkTextOveflow(e)) {
      return;
    }
    console.log('sdsd');
    const detailModal = document.getElementById('title_bar');
  ...
  function clearDetailFromHover(e) {
    if (!checkTextOveflow(e)) {
        return;
      }
    document.querySelector('.frequent_detail').remove();
  }

  const frequent = document.getElementById('title');
  frequent?.addEventListener('mouseover', getDetailFromHover);
  frequent?.addEventListener('mouseout', clearDetailFromHover);
});