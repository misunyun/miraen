...
Module.common = (function () {
	var _container;
	var _iframe_container_id = undefined;
	var _mouse_wheel_enabled = false;
	/* iframe */
	var _size = { w: 0, h: 0 };
	var _pos = { x: 0, y: 0 };
	var _zoom_target = { x: 0, y: 0 };
	var _zoom_point = { x: 0, y: 0 };
	var _scale = 1;
	var zoom_factor = 0.1;
	var _max_scale = 3;
	var _delta;
	var previous_mouse_move_event;
	var is_dragging;
	function _set_title(idx) {
		var obj = viewer._playlist_data.playlist[idx];
		var corner = obj.corner === null ? '' : obj.corner
	...
	function _is_samesize(container) {
		...
		var content_type = $("#content-scale").attr('type');
		if (content_type) {
			if (content_type == 'image') {
				var img = $("#content-scale .carousel-item.active").children("img").first();
				if (img && img.length >= 1) {
		...
		if (content_type) {
			...
			if ($("#pdf-iframe").length > 0) {
				...
				try {
					...
					var pdfview = viewer.getPageView(0);
					var viewbox = pdfview.viewport.viewBox;
					// console.log(pdfview);
					if (pdfview && viewbox[2] > 0 && viewbox[3] > 0) {
						// var scale = pdfview.scale;
						width = viewbox[2];
						height = viewbox[3];
						// console.log(width, height);
	...
	function _fit_height() {
		var target = $('#module-body');
		target.removeAttr('style');
		if (target.offset()) {
			var footerHeight = 38;
			var targetTop = target.offset().top;
			var obj = $('#viewer-hd');
			var targetHeight = obj.offset().top - (targetTop + (MARGIN * 2));
	...
	function _get_zoom() {
		if (_container) {
			var smartData = _container.data('smartZoomData');
	...
	function _iframe_zoom(container, max_scale) {
		console.log(container, max_scale);
		if (container) {
	...
	function _iframe_current_position(type, e) {
		console.log('_iframe_current_position', type);
		if (!_container) {
			return;
		}
		console.log('_iframe_current_position', _container);
		console.log('_iframe_current_position', _container.offset());
		...
		// var _style = _container.attr('style');
...
Module.image_viewer = (function () {
	...
	function _drow_image(id, image_list, user_margin) {
		if (id && image_list) {
			...
			$.each(image_list, function (index, item) {
				var $carousel_item = $('<div class="carousel-item align-items-center flex-column p-4"></div>');
				var $image = $('<img class="d-block">');
				$image.attr('src', item);
			...
			*/
			...
			var page = _paging(_image_list.length, _cur_idx);
	...
	function _prev_image(img) {
		var $carousel_control_prev = $('.carousel-control-prev').children(".carousel-control-prev-icon");
	...
	function _next_image(img) {
		var $carousel_control_next = $('.carousel-control-next').children(".carousel-control-next-icon");
...
Module.video_viewer = (function () {
	...
	function _drow_video(id, type, _cb) {
		if (id) {
			...
			if (player_type === 'audio') {
				$video = $('<audio id="video" class="video-js"></audio>');
			} else {
	...
	function _video_control(_cb) {
		if ($('.video-js').length) {
			...
			var videojs_control_bar_children = [
				'playToggle',
				'stopButton',
				'progressControl',
				'volumeMenuButton',
...
Module.pdf_viewer = (function () {
	var drawing_canvas = undefined;
	var _image_list = []
	var _pdf_url = undefined;
	var _cur_idx = 0;
	function _drow_pdf(id, pdf_url, start_page, image_list, user_margin) {
		console.log("_drow_pdf", pdf_url, start_page);
		if (id && image_list) {
			_image_list = image_list;
		}
		if (id && pdf_url) {
			...
			$pdf_iframe = $('<iframe id="pdf-iframe" width="100%" height="100%"></iframe>');
			// $pdf_iframe.on('load', function (e) {
			// 	_set_drawing_canvas();
			// });

			// var iframe_url = '/assets/viewer/pdf/web/pdf.html?file=' + pdf_url + '&page=' + start_page;
			// var iframe_url = '/assets/viewer/pdf/web/pdf.html?file=' + pdf_url + '&page=' + start_page;  //로컬 테스트용
			// var iframe_url = '/js/pc/ele/viewer/pdf/web/pdf.html?file=' + pdf_url + '.pdf'+ '&page=' + start_page;  //로컬 테스트용
			// var iframe_url = '/assets/viewer/pdf/web/pdf.html?file=' + 'https://contents.m-teacher.co.kr/cms/smartppt/auth/ebook/subjectContents/76c85459bec448a7b12df8c93ca5918d.pdf'+ '&page=' + start_page;  //로컬 테스트용
			// var iframe_url = '/js/pc/ele/viewer/pdf/web/pdf.html?file=' + 'https://contents.m-teacher.co.kr/cms/smartppt/auth/ebook/subjectContents/76c85459bec448a7b12df8c93ca5918d.pdf'+ '&page=' + start_page;  //로컬 테스트용
			let iframe_url;
			if (start_page === "link") {
				iframe_url = pdf_url
			} else {
	...
	function _set_drawing_canvas_to_target_id(target_id) {
		var $drawing_control_parent = $("#pdf-iframe").contents().find("#" + target_id);
		if (!$drawing_control_parent || $drawing_control_parent.length < 1) {
			return;
		}

		var $drawing_control = $("#pdf-iframe").contents().find("#drawing-control");
		...
		if (!drawing_canvas) {
			var _container = $drawing_control.get(0);
