    ...
    let screen = {
        ...
        f: {
            ...
            setContentsTabDisplayList: () => {
                ...
                screen.v.contentsTabCategoryList.map((data, k) => {
                    let res = `
                        <a href="${data?.viewPageYn === 'Y' ? data.pageUrl + '/' + data.masterSeq : data.pageUrl}" 
                           data-contents="tab" 
                           data-rightyn="${data?.listPageUseYn}" 
                           data-rights="${data?.listPageRight}">
                ...
                screen.v.contentsTabCategoryList.map((data, k) => {
                    let res = `
                        <a href="javascript:void(0);" 
                           data-name="${screen.v.contentsTabCategoryType}" 
                           data-id="${data?.referenceFileId}" 
                           data-seq="${data.referenceSeq}" 
                           data-type="${data?.referenceFileUploadMethod}" 
                           data-cmstype="${data?.duration ? 'Y' : 'N'}"
                           data-url="${data.referencePath}">
                            <div class="image-wrap">
            ...
            getCheckRightMainRecommendData: (e) => {
                ...
                if(screen.v.right === "0010"
               && (screen.v.contentsTabCategoryType === "koreanBasic" || screen.v.contentsTabCategoryType === "koreaMusic" || screen.v.contentsTabCategoryType === "todayClass")) {
        ...
        event: function () {
            ...
            $('#mainTeacherAuthInfoButton').off('click').on('click', () => {
                ...
                window.open("https://e.m-teacher.co.kr/pages/award/mjmirae11.mrn", "_blank");
            });
