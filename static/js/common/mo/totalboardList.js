...
let screen = {
  v: {
    ...
    resultList: [],
    categoryList: [],
    mainBoardInfo: $('[name=jsMainBoardInfo]').val() ? JSON.parse($('[name=jsMainBoardInfo]').val() ?? '') : {}
  },
    ...
    getEleGnbList: function(categoryCode){
      ...
      $cmm.ajax({
        ...
        success: function (res) {
          ...
          //모바일 하위 있을때 상위 배너
          if (screen.v.mainBoardInfo?.bannerUseYn === 'Y' && !!screen.v.mainBoardInfo?.moBannerFilePath && !!location.pathname.includes('tabList')) {
            lv2_section.after(`
                <section class="banner" style="padding-bottom: 12px; border-bottom: 1px solid #e6e6e6" >
                      <img style="width: 100%" src="${screen.v.mainBoardInfo?.moBannerFilePath}" alt="${screen.v.mainBoardInfo?.moBannerDisplayName ?? 'banner-img'}">
                </section>
            `);
          }
        }
        ...
        const htmlButtons = `
          <div class="board-buttons">
            ...
            <button
                type="button"
                name="shareBtn"
                class="${(screen.v.mainInfo.viewPageYn === 'N' && data.videoFileUploadMethod != 'LINK' && data.videoFileUploadMethod != '') ? 'display-hide' : 'icon-button size-sm '+shareHide}"
                data-path="${data.videoPath}"
