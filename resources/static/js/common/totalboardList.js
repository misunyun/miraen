    ...
    c: {
        ...
        getTotalboardSearchList: (pagingNow, isCateReload, selectedLevel)=> {
            ...
            const options = {
                ...
                success: function (res){
                    //영상 필터 숨김 처리
                    if (screen.v.mainInfo.videoUseYn == 'Y') {
                        $('#searchVideoTime').next("span").show();
                    } else {
                        $('#searchVideoTime').next("span").hide();
                    }
        ...
        `;
          ...
          <div class="board-buttons">
            ...
            <div class="buttons">
              ...
              <button type="button" 
                      name="shareBtn" 
                      class="${(screen.v.mainInfo.viewPageYn === 'N' && data.videoFileUploadMethod != 'LINK' && data.videoFileUploadMethod != '') ? 'display-hide' : 'button type-icon size-sm '+shareHide}"
                      data-path="${data.videoPath}"
