  ...
  let screen = {
    ...
    f: {
      ...
      idEmailFormChk: (id, alertYn)=> {
        ...
        if($text.isEmpty(val)) return false;
      ...
      idFormChk: (val, id, alertYn)=> {
        ...
        if(!returnFlag){
          ...
          $.ajax({
            type:"POST",
            data:{'userId':val},
            url:"/User/duplicateCheckId.ax",
            success: function(res){
              if(res.status == "duplicated"){
                returnFlag = false;
                screen.f.setErrorMsg(id, 200, 400, res.dispMessage, alertYn);
              }
            },
            error: function(e){
              console.log(e);
    ...
    event: function (){
      ...
      $(document).on('keyup', '#user-id', screen.f.keypress1);
    ...
    init: function (){
      console.log('common auth mo js init!');
      screen.event();
	},
  };
